...
| Membership       | `["membership", tree_id, id_commitment]`    | Per-identity (leaf_index, rate_limit, expiry timestamps, holder, deposit) — single-use |
...
**Initialize** — Writes the config PDA: merkle program ID, tree ID, price, treasury, rate-limit cap and membership durations. Setup is still split across two transactions (a fused init exceeds the execution-gas cap): `InitializeMerkleTree` chains to the merkle program to initialize the tree.
...
One caveat on the refund: `Register` records the paying account id as the membership's `holder`, and a private payer's id is derived from its keys. A *public* `Erase` would credit that id in public state, where nothing can authorize a debit from it — so a private payer must issue its `Erase` as a privacy-preserving transaction too, or the deposit is unrecoverable. The shipped host tools only build public transactions, so this path is exercised by `register_from_a_private_payer` (feature `rc5-state-tests-privacy`) and not by any binary.
...
**Extend** — Renews an existing membership from inside its grace period. **Holder only, and free**: the escrowed deposit is what pays for the slot, and it stays escrowed for as long as the membership lives, so a renewal moves no value. Restricting it to the holder is what makes free renewal safe — a third party who could renew indefinitely would pin an abandoned membership's share of the rate-limit budget forever, and an abandoned membership has no holder left to renew it, so it lapses and becomes erasable.

**ForceExpire** — Brings a membership's grace period forward to now (`min(grace_start, now)`, so it can never postpone expiry), letting the holder exit ahead of its active period. Holder only, and reversible only by the holder: `Extend` pushes the expiry back out, and nobody else may call it.

The deposit is not released here. The leaf stays in the tree until `Erase`, so the grace period is also the interval in which `Slash` can still forfeit the deposit — an exit that released funds immediately would outrun slashing. That makes `grace_period_duration` the collateral window, which is why `Initialize` refuses a zero one.
...
**An `id_commitment` is single-use for the life of the tree.** Erase empties the membership PDA but cannot un-own it — LEZ never returns an account to its default state — and the `init` constraint that makes registration one-shot therefore makes it permanent. The same holds after `Slash`. Re-entering the registry means a fresh identity.
