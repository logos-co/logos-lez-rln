...
mod tests {
    ...
    fn build_token_create_tx(
        ...
        PublicTransaction::new(
            message.clone(),
            WitnessSet::for_message(&message, &[supply_holder_key])
        )
    }
    ...
    fn build_token_transfer_tx(
        ...
        PublicTransaction::new(
            message.clone(),
            WitnessSet::for_message(&message, &[from_key])
        )
    }
    ...
    /// Builds a transaction to initialize the RLN registration program.
    ///
    ...
    fn build_registration_init_tx(
        ...
        build_registration_init_tx_with_config(
            registration,
            merkle,
            tree_id,
            payment_token_id,
    ...
    /// Builds a transaction to initialize the RLN registration program with custom config.
    ...
    fn state_with_initialized_registration_config(
        ...
        // 2. Create payment token via token program
    ...
    fn test_registration_init_succeeds() {
        ...
        let init_tx = build_registration_init_tx(
            &registration,
            &merkle,
            &TREE_ID,
            &payment_token_id,
    ...
    fn test_token_create_succeeds() {
        ...
        let definition_id = AccountId::new([10; 32]);
    ...
    fn test_token_transfer_succeeds() {
        ...
        let (_to_key, to_id) = create_test_keypair(2);
        let definition_id = AccountId::new([10; 32]);
    ...
    fn test_register_succeeds() {
        ...
        let register_tx = build_register_tx(
            ...
            id_commitment,
            rate_limit,
            0, // user's nonce (first tx from this account on registration program)
            0, // next_index
        );
    ...
    fn test_private_credit_flow() {
        ...
        let (transfer_tx, _credit_sender_post, private_credits_2) = private_token_transfer(
            ...
            credit_amount,
            0xdd, // sender new nonce
            0xee, // recipient nonce
            &setup.state,
        );
        setup.state
