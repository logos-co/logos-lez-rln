...
(e.g. Nim procs in logos-delivery) by calling `liblogos_rln_membership_module`
...
| `generate_proof(scope, signal)` | `generate_proof(registry_id, rln_identifier_hex, signal_hex)` | result | RateLimitProof (below) + `"message_id"`, `"epoch"`, `"membership_hash"` |
...
| `get_epoch_quota(scope)` | `get_epoch_quota(registry_id, rln_identifier_hex)` | result | `{"epoch_index":N,"rate_limit":N,"remaining":N}` — one epoch observation, purely local; `epoch_index` is a NUMBER (`floor(unix/epoch_size)`, what a QuotaProvider's `epochIndex` consumes); `no_usable_membership` is the wall-clock-fallback cue |
